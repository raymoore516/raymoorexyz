...
Idempotent statements address SQL execution, but Flyway also needs to adopt a nonempty schema without its history table. For the first production attachment, `application.yml` temporarily sets `spring.flyway.baseline-on-migrate=true` and explicitly sets `spring.flyway.baseline-version=0`. That records a starting baseline below `V1`, so the idempotent `V1` still executes. The usual baseline version of `1` would skip `V1`. Return `baseline-on-migrate` to `false` immediately after the successful first production startup; subsequent starts use the history table and do not need automatic baselining. Fresh local databases do not require a baseline. See [Flyway baseline-on-migrate](https://documentation.red-gate.com/flyway/reference/configuration/flyway-namespace/flyway-baseline-on-migrate-setting).
