...
This is the implementation plan for AI coding agents. The database foundation is now implemented: Java 25, Spring Boot 4.1.1, Maven Wrapper 3.9.11, local PostgreSQL 15.19 through Compose, Flyway V1, Spring Data JDBC `Contestant` and `Pick` records, and a command-line contestant lookup using `ContestantRepository.findAll()`. HTTP and frontend features remain planned. It defines shared technologies, architecture, and authentication for the personal website. Project-specific business requirements and data models live in `.projects/<project>/README.md`. Proposed API contracts and unresolved business rules are labeled explicitly.
...
- Choose and pin Node and frontend dependency versions during frontend scaffolding. Java 25, Spring Boot 4.1.1, Maven 3.9.11, and PostgreSQL 15.19 are selected. The implementation must match the setup examples and document tested versions.
