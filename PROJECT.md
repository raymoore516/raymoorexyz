...
- Start with a normal Vite React/TypeScript SPA. The initial static home page renders directly through `App` without a router. Add a small client router when introducing additional pages, defining `/` and the routes specified by each project plan; keep routing and API fetching explicit, and add broader state/query libraries only when their benefit is clear.
...
The current frontend increment renders only this content at `/`, without navigation, API calls, or a database dependency:
...
Keep this in `HomePage.tsx`, rendered by `App.tsx`. Vite uses fixed port 5173 for development and 4173 for build preview. A Spring controller is unnecessary for this page; serving the built assets through Spring Boot remains a separate integration step. Add shared navigation with subsequent frontend work; a personal profile, project dashboard, and richer home-page content are later work.
