...
Build a personal website as a monolithic web application in a monorepo. The first project is [Madison SC](.agents/projects/madisonsc/PROJECT.md), an NFL pick'em league tracker. A shared, responsive hamburger navigation bar is also part of the initial website. Future features may include an arcade or blog.

This is a greenfield implementation plan for AI coding agents. It describes the intended application, not completed functionality. It defines shared technologies, architecture, and authentication for the personal website. Project-specific business requirements and data models live in `.agents/projects/<project>/PROJECT.md`. Proposed API contracts and unresolved business rules are labeled explicitly.
...
This file is the shared technical plan. The [Madison SC plan](.agents/projects/madisonsc/PROJECT.md) owns its league rules, contestant/pick schema, API contracts, and UI requirements. [README.md](README.md) describes developer setup and day-to-day workflows. Keep these documents consistent as decisions are made.
...
- Future projects, such as an arcade or blog, share this application and have their own plans under `.agents/projects/`.
...
- Configure Vite to proxy `/api` to Spring Boot. React owns page rendering and fetches data from the JSON API. Local secret-authenticated administrator requests go directly to Spring Boot at `http://localhost:8080` from an API client; the public React application never receives the API secret.
...
│   └── projects/
│       └── madisonsc/
│           └── PROJECT.md          # Madison SC requirements and data model
├── frontend/
│   ├── package.json
│   ├── package-lock.json
│   ├── vite.config.ts
...
Use one `raymoorexyz` PostgreSQL database per environment. Each project gets its own schema within that database; for example, Madison SC owns `madisonsc`. Project tables, constraints, and domain mappings are documented in that project's plan, starting with the [Madison SC database model](.agents/projects/madisonsc/PROJECT.md#madison-sc-domain-and-database).
...
- Start with a normal Vite React/TypeScript SPA and a small client router. Define `/` for the home page and add the routes specified by each project plan; keep routing and API fetching explicit, and add broader state/query libraries only when their benefit is clear.
...
- Use JUnit for business rules and validation, with acceptance cases defined in project plans.
...
- Use Vitest and React Testing Library for meaningful UI behavior, particularly request failures, empty states, and hamburger menu keyboard/navigation behavior. Test user-visible behavior rather than internal component details.
