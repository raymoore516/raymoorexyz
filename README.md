...
The database foundation is implemented: local PostgreSQL through Docker Compose, Flyway migrations, and a Spring Boot command-line application that queries contestants and exits. HTTP endpoints and the frontend are still planned.
...
3. Export the same settings for Java, then run the application:
   ...
   `App.java` then executes `SELECT * FROM madisonsc.contestant`, prints each row using `System.out.println`, and exits successfully. A fresh database prints `Found 0 contestant(s).`; migrations intentionally contain no sample contestants. The table is singular and schema-qualified, as specified in the project plan.
...
`@SpringBootApplication` enables Spring's automatic configuration. The Spring Data JDBC starter supplies the connection pool and `JdbcTemplate`, which handles JDBC connection/resource management. The `@Bean` method registers a `CommandLineRunner`: Spring supplies its `JdbcTemplate` argument and runs it after startup migrations finish. No repository or entity classes are needed for this first query. Flyway integration follows [Spring Boot's database initialization guidance](https://docs.spring.io/spring-boot/how-to/data-initialization.html).
